...
function loadAttachments (ddoc, dir) {
  ...
  files.forEach(function (f) {
    ...
    ddoc._attachments[f] = {filename:filename, data:fs.readFileSync(filename), mime:mime}
  })
}
...
function loadFiles (ddoc, dir) {
  ...
  files.forEach(function (f) {
    ...
    for (var i=0;i<s.length;i+=1) {
      if (i == (s.length - 1)) {
        obj[s[i]] = fs.readFileSync(f).toString();
...
function loadModules (ddoc, dir) {
  ...
  files.forEach(function (f) {
    ...
    for (var i=0;i<s.length;i+=1) {
      if (i == (s.length - 1)) {
        obj[s[i].slice(0, s[i].length - path.extname(f).length)] = fs.readFileSync(f).toString();
      } else if (s[i].length != 0){
        if (!obj[s[i]]) {
          obj[s[i]] = {};
        }
