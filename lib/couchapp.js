...
function loadAttachments (ddoc, dir) {
  ...
  files.forEach(function (f) {
    ddoc._attachments.push([f, path.join(dir, f), mimetypes.lookup(path.extname(f).slice(1))])
  })  
  // files.forEach(function (f) {
...
function loadFiles (ddoc, dir) {
  ...
  ddoc._load = function () {loadFiles(ddoc, dir)}
  ddoc._load.files = files;
}
...
function loadModules (ddoc, dir) {
  ...
  ddoc._load = function () {loadModules(ddoc, dir)}
  ddoc._load.files = files;
}
